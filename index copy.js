...
client.on("messageCreate", async (message) => {
    ...
    if(message.content.startsWith(PREFIX))
    {
        ...
        if(cmd.startsWith('p') && cmd != 'pause') 
        {
            ...
            try
            {
                const { createAudioPlayer, createAudioResource, StreamType, AudioResource, NoSubscriberBehavior } = require('@discordjs/voice');
                    ...
                    urlQ = message.content.substring(3)
                    const r = await yts(urlQ)
                    ...
                    videos.forEach( function ( v ) {
                        ...
                        songName = `${v.title} (${v.timestamp})`
                    } )
                    console.log(videos[0].url)
    
                    var finalUrl
                    finalUrl = videos[0].url
                    ...
                    outputFs.on('close', () => {
                        ...
                        const resource = createAudioResource('/Users/atiksh/Coding2021/cory_krelove/test_song_for_ytdl.mp4', { inlineVolume: true });
        ...
        if(cmd == "pause")
        {
            ...
            {
                message.channel.send("I aint playning anythign")
            }
        }
        ...
        if(cmd.startsWith("dc"))
        {
            ...
            {
                message.channel.send("I ain't in a vc you shit")
            }
        ...
        if(cmd.startsWith("rolegive"))
        {
            ...
            var personK = message.guild.members.cache.get(person.id)
            if(message.author.id == 672254825295249408 || message.author.id == '768483086689370182')
            { 
               try
               {
                    ...
                    user.roles.add('744243983445655623')
               }
               catch
               {
